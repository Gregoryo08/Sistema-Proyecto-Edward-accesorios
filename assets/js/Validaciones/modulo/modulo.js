...
window.validarModuloAntesDeConfirmar = function (id) {
  const $input = $(id);
  const valor = $input.val().trim();
  const $feedback = $("#error_" + id.substring(1));
  const esValido = valor.length >= 4 && valor.length <= 30 && /^[A-Za-zÁÉÍÓÚáéíóúÑñÜü\s-]+$/.test(valor);
  ...
  if (!valor) {
    ...
    $feedback.text("El nombre del módulo es obligatorio.").show();
    Swal.fire({ title: "Nombre requerido", text: "Indique el nombre del módulo.", icon: "warning", color: "white", background: "#000910" });
    return false;
  }

  if (!esValido) {
    $input.addClass("is-invalid");
    $feedback.text("El nombre debe tener entre 4 y 30 letras.").show();
    return false;
  }
...
$(document).ready(function () {
    ...
    $("#registro").click(function () {
      if (validarModuloAntesDeConfirmar("#modulo")) {
    ...
    $("#modificar").click(function () {
      if (validarModuloAntesDeConfirmar("#modulo_modificar")) {
