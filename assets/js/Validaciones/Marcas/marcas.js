...
$(document).ready(function () {
  ...
  function validarMarca(id, feedbackId) {
    const $input = $(id);
    const valor = $input.val().trim();
    const $feedback = $(feedbackId);
    const esValido = valor.length > 2 && /^[a-zA-ZñÑáÁéÉíÍóÓúÚüÜ\-\s]+$/.test(valor);
    ...
    if (!valor) {
      ...
      $feedback.text("El nombre de la marca es obligatorio.").show();
      Swal.fire({
        title: "Nombre requerido",
        text: "Indique el nombre de la marca.",
        icon: "warning",
        color: "white",
        background: "#000910"
      });
    ...
    if (!esValido) {
      $input.addClass("is-invalid");
      $feedback.text("Ingrese un nombre válido de más de 2 caracteres.").show();
      return false;
    }
  ...
  $("#btnRegistrarMarca").click(function () {
    if (validarMarca("#nombre", "#texto_mensaje_nombre")) {
  ...
  $("#btnModificarMarca").click(function () {
    if (validarMarca("#nombreModificar", "#texto_mensaje_nombre_modificar")) {
