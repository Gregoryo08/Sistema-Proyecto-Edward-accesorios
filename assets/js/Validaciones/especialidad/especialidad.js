...
$(document).ready(function () {
  ...
  function validarEspecialidad(id, feedbackId) {
    const $input = $(id);
    const valor = $input.val().trim();
    const $feedback = $(feedbackId);
    const esValido = valor.length >= 4 && /^[A-Za-zÁÉÍÓÚáéíóúÑñÜü\s-]+$/.test(valor);
    ...
    if (!valor) {
      ...
      $feedback.text("El nombre de la especialidad es obligatorio.").show();
      Swal.fire({
        title: "Nombre requerido",
        text: "Indique el nombre de la especialidad.",
        icon: "warning",
        color: "white",
        background: "#000910"
      });
    ...
    if (!esValido) {
      $input.addClass("is-invalid");
      $feedback.text("El nombre debe tener al menos 4 letras.").show();
      return false;
    }
  ...
  $("#registro").click(function () {
    if (validarEspecialidad("#especialidad", "#mensajeEspecialidad")) {
  ...
  $("#modificar").click(function () {
    if (validarEspecialidad("#especialidad_modificar", "#mensajeEspecialidadModificar")) {
