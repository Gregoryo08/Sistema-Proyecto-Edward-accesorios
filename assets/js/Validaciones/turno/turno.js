...
function registrarTurno() {
    ...
    const dataToSend = {
        fecha: fecha,
        cedulas_persona: cedulas_empleados,
        hora_entrada: hora_e,
        hora_salida: hora_s,
        obs: obs,
        accion: accion,
    ...
    if (idTurnoModificar) {
        dataToSend.id_turno = idTurnoModificar;
    }
        ...
        if (result.dismiss === Swal.DismissReason.timer) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    } else if (response.error || response.invalido || response.incompleto) {
                        
                        mensaje("error", response.error || response.invalido || "Datos incompletos");
                    }
                },
                error: function(xhr, status, error) {
                    mensaje("error", "Ha ocurrido un error en el servidor!");
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.error) {
                ...
                empleadosAsignadosMod = new Map(datos_empleados.map(emp => [emp.cedula_persona, {
                    cedula: emp.cedula_persona,
                    nombre: emp.nombre,
                    apellido: emp.apellido,
                    cargo: emp.nombre_cargo,
                }]));
                
                observacionesRegistradasMod = Array.isArray(datos_observaciones) 
                    ? datos_observaciones.map(obs => obs.descripcion) 
                    : [];
                
                if (observacionesRegistradasMod.length > 0 && typeof observacionesRegistradasMod[0] === 'string' && observacionesRegistradasMod[0].includes('-')) {
            ...
            success: function(response) {
                if (response.error || response.invalido || response.incompleto) {
                    ...
                    $.each(datos_empleados, function(i, empleado) {
                        ...
                        newRow.insertCell(0).textContent = empleado.cedula_persona;
