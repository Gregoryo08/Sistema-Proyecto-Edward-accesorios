   ...
   public function listarEmpleados()
{
    ...
    $sql = "SELECT e.cedula_persona as id, 
                   CONCAT(p.nombre, ' ', p.apellido) as text, 
                   p.nombre, 
                   p.apellido, 
                   c.nombre_cargo as cargo 
            FROM empleados as e 
            INNER JOIN persona as p ON e.cedula_persona = p.cedula_persona 
            INNER JOIN cargos as c ON e.id_cargo = c.id_cargo 
    ...
    {
        $stmt = $this->prepare("SELECT t.fecha_turno, t.hora_entrada, t.hora_salida, t.id_turno, COUNT(te.cedula_persona) as conteo FROM turnos as t LEFT JOIN turno_empleado as te ON t.id_turno = te.id_turno GROUP BY t.id_turno ORDER BY t.fecha_turno ASC");
        $stmt->execute();
    ...
    public function consultar()
{
    ...
    try {
        ...
        $stmt = $this->prepare("SELECT e.cedula_persona, p.nombre, p.apellido, c.nombre_cargo 
                                FROM turno_empleado as te 
                                INNER JOIN empleados as e ON te.cedula_persona = e.cedula_persona 
                                INNER JOIN persona as p ON e.cedula_persona = p.cedula_persona 
                                INNER JOIN cargos as c ON e.id_cargo = c.id_cargo 
                                WHERE te.id_turno = :id");
        if (!($stmt->execute([":id" => $id]))) {
            return ["error" => "Ah ocurrido un error con el servidor!"];
        }
   ...
   public function registrar()
{
    ...
    try {
        ...
        if (!empty($cedulas_validas)) {
            $stmt_empleados = $this->prepare("INSERT INTO turno_empleado (id_turno, cedula_persona) VALUES (:idT, :c)");
