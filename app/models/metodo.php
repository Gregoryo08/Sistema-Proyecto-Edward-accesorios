...
class metodo extends Conexion
{
    ...
    public function registrar()
    {
        if ($this->getNombreMetodoPago() === null || $this->getMoneda() === null || $this->getMensaje() !== null) {
            ...
            } elseif ($this->getMoneda() === null && $this->getMensaje() === null) {
                $this->setMensaje("La moneda no ha sido establecida.");
            }
        ...
        $estado = $this->getEstado() ?? 1; // 1 por defecto (Activo)
        ...
        try {
            ...
            $query = $this->prepare("INSERT INTO metodo_pago(nombre_metodopago, moneda, cuenta, estado) VALUES (:nombre_metodopago, :moneda, :cuenta, :estado)");
            $query->bindParam(":nombre_metodopago", $nombre);
            $query->bindParam(":moneda", $moneda);
    ...
    public function modificar()
    {
        ...
        try {
            ...
            $query = $this->prepare("UPDATE metodo_pago SET nombre_metodopago = :nombre_metodopago, moneda = :moneda, cuenta = :cuenta, estado = :estado WHERE id_metodopago = :id_metodopago");
            $query->bindParam(":nombre_metodopago", $nombre);
            $query->bindParam(":moneda", $moneda);
            $query->bindParam(":cuenta", $cuenta);
            $query->bindParam(":estado", $estado, PDO::PARAM_INT);
    ...
    public function setMoneda($moneda)
    {
        ...
        if ($moneda !== 'VES' && $moneda !== 'USD') {
            $this->setMensaje("Moneda no permitida. Solo se acepta VES o USD.");
            $this->moneda = null;
            return false;
        }
        $this->moneda = $moneda;
