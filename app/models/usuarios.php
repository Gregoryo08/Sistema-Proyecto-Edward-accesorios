...
class Usuarios extends Empleados {
    ...
    public function validarSeguridad()
    {
        ...
        $stmt = $conexUser->prepare("SELECT COUNT(*) as conteo FROM usuarios WHERE cedula_usuario = :c");
    ...
    public function crearPerfil()
    {
        ...
        try {
            ...
            $stmt = $conexUsuario->prepare("INSERT INTO usuarios(cedula_usuario, clave, id_rol) VALUES (:cedula, :clave, :id_rol)");
            if (!($stmt->execute([":cedula" => $usuario, ":clave" => $hash, ":id_rol" => $rol]))) {
                $conexUsuario->rollBack();
                return ["error" => "Error al insertar en la tabla usuarios."];
            }

            $stmt2 = $conexSistema->prepare("UPDATE empleados SET perfil = 'si' WHERE cedula_persona = :cedula");
            if (!($stmt2->execute([":cedula" => $usuario]))) {
                $conexUsuario->rollBack();
                return ["error" => "Error al actualizar perfil en empleados."];
            }
    ...
    public function cambiarEstatus()
    {
        ...
        try {
            ...
            $stmt = $conexSistema->prepare("UPDATE empleados SET perfil = :p WHERE cedula_persona = :c");
    ...
    public function consultarSuspendido()
    {
        try {
            $conex = new Conexion("sistema");
            $stmt = $conex->prepare("SELECT * FROM empleados WHERE perfil = 'suspendido'");
            $stmt->execute();
